...
class Tokenizer:
    ...
    def build_vocab(self):
        ...
        tokens = [
            self.clean_word(word) for word in tokens if word
        ]  # Clean and filter out empty words
        vocab = list(
            {
                word
                for word in tokens
                if self.freq_dist.get(word, 0) > self.freq_threshold
            }
        )
        return vocab
    ...
    def encode(self, sentence):
        ...
        words = [
            self.clean_word(word) for word in words if word
        ]  # Clean and filter out empty words
